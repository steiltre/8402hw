    ...
    \begin{solution}
      \begin{enumerate}
        \item
          ...
          As discussed in Section 4.3.3, our solution is then
          \[ \psi(x) \sim
            \begin{cases}
              \frac{1}{|q(x)|^{1/4}} \left[ 2 a_R \cos \left( \frac{1}{\varepsilon} \theta_1(x) - \frac{\pi}{4} \right) + b_R \cos \left( \frac{1}{\varepsilon} \theta_1(x) + \frac{\pi}{4} \right) \right] &\text{ if } x < a \\
              \frac{1}{q(x)^{1/4}} \left( A_L e^{\theta_2(x)/\varepsilon} + B_L e^{-\theta_2(x)/\varepsilon} \right) &\text{ if } a<x<b \\
              \frac{1}{|q(x)|^{1/4}} \left[ 2 B_L \cos \left( \frac{1}{\varepsilon} \kappa_2(x) - \frac{\pi}{4} \right) + A_L \cos \left( \frac{1}{\varepsilon} \kappa_2(x) + \frac{\pi}{4} \right) \right] &\text{ if } b < x
          \end{cases} \]
    ...
    \begin{solution}
      ...
      \begin{enumerate}
          \item
            ...
            with the boundary conditions $u_0 = 0$ for $y = \pm 1$.
            ...
            \begin{equation} \label{eq:u_0}
               u_0(x,y) = A(x) \sin \left[ \lambda(y + 1) \right] ,
            \end{equation}
            where we must have $2 \lambda = n \pi$ in order to satisfy the boundary conditions. Thus we have
            ...
            with the boundary condition $u_1 = 0$ at $y = \pm 1$.

            To get our solvability condition, we multiply by $u_0$ and integrate from -1 to 1 in the $y$ direction. This gives
            \[ \int_{-1}^{1} u_0 (u_{1,yy} + \lambda_n^2 u_1) dy = i \int_{-1}^{1} u_0 (\theta_xx u_0 + 2 \theta_x u_{0,x} + \omega \alpha u_0) dy \]
            For the left-hand side, integrating by parts twice leaves
            \begin{align*}
              \int_{-1}^{1} u_0 (u_{1,yy} + \lambda_n^2 u_1) dy
              &= \int_{-1}^{1} u_1 ( u_{0,yy} + \lambda_n^2 u_0) dy \\
              &= 0 \quad \parbox{4cm}{by the eikonal equation}
            \end{align*}

            So we have
            \begin{align*}
              0 &= \int_{-1}^{1} u_0 (\theta_xx u_0 + 2 \theta_x u_{0,x} + \omega \alpha u_0) dy \\
              &= \int_{-1}^{1} (\theta_x u_0^2)_x dy + \omega \alpha \int_{-1}^{1} u_0^2 dy \quad \parbox{4cm}{($\omega, \alpha$ are constant in $y$)} \\
              &= \frac{d}{dx} \int_{-1}^{1} \theta_x u_0^2 dy + \omega \alpha A^2 \int_{-1}^{1} \sin^2 \left( \frac{n \pi}{2}(y+1) \right) dy \quad \parbox{4cm}{from equation \eqref{eq:u_0}} \\
              &= \frac{d}{dx} \theta_x \int_{-1}^{1} u_0^2 dy + \omega \alpha A^2 \\
              &= \frac{d}{dx} (\theta_x A^2) + \omega \alpha A^2
            \end{align*}

            Therefore, we have
            \[ (A^2)' \theta_x + (\theta_{xx} + \omega \alpha) A^2 = 0 \]
            Rewriting, we have
            \[ \frac{(A^2)'}{A^2} = - \frac{\theta_{xx} + \omega \alpha}{\theta_x} .\]
