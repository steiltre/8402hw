...
\begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \item
          ...
          \begin{align*}
            ...
            &= f \left( y_0, \frac{\partial y_0}{\partial t_1} + \varepsilon \frac{\partial y_0}{\partial t_2} \right) + O(\varepsilon) \\
            &= f \left( y_0, \frac{\partial y_0}{\partial \tau} \right) + O(\varepsilon)
          \end{align*}
          ...
          \begin{align*}
            \frac{\partial^2 y_0}{\partial t_1 \partial t_2} &= \frac{\partial^2}{\partial t_1 \partial t_2} \big( A(t_2) \cos(t_1 + \phi(t_2) \big) \\
          ...
          \cos \tau \right) - f \left( y_0, \frac{\partial y_0}{\partial \tau} \right) .\]
        ...
        \item
          ...
          \[ 0 = \bigintss_{0}^{2 \pi} \left[ 2 \left( \frac{\partial A}{\partial t_2} \sin \tau + A \frac{\partial \phi}{\partial t_2}
          \cos \tau \right) - f \left( y_0, \frac{\partial y_0}{\partial \tau} \right) \right] \sin \tau d\tau \]
          and
          \[ 0 = \bigintss_{0}^{2 \pi} \left[ 2 \left( \frac{\partial A}{\partial t_2} \sin \tau + A \frac{\partial \phi}{\partial t_2}
          \cos \tau \right) - f \left( y_0, \frac{\partial y_0}{\partial \tau} \right) \right] \cos\tau d\tau \]
          ...
          \[ \frac{\partial A}{\partial t_2} = \frac{1}{2 \pi} \int_{0}^{2 \pi} f \left( y_0, \frac{\partial y_0}{ \partial \tau} \right) \sin \tau
          d\tau \]
          and
          \[ A \frac{\partial \phi}{\partial t_2} = \frac{1}{2 \pi} \int_{0}^{2 \pi} f \left( y_0, \frac{\partial y_0}{ \partial \tau} \right) \cos
          \tau dt_1 \]


        \item
          We have $f = (y^4-1)y'$. By part (a), we know $y_0 = A(t_2) \cos \tau$. We also have the conditions from part (c) that we must satisfy:
          \begin{align*}
            A' &= - \frac{1}{2 \pi} \int_{0}^{2 \pi} \left( A^4 \cos^4 \tau - 1 \right) A \sin^2 \tau d\tau \\
            &= - \frac{1}{2 \pi} \int_{0}^{2 \pi} ( A^5 \cos^4 \tau - A ) \sin^2 \tau d \tau \\
            &= - \frac{A^5}{16} + \frac{A}{2}
          \end{align*}

          The solution to this differential equation is
          \[ A(t_2) = \frac{2^{3/4}}{(C \exp(-x/2) + 1)^{1/4}} \]
          for some constant $C$.

          We also have the condition
          \begin{align*}
            A \varphi' &= - \frac{1}{2 \pi} \int_{0}^{2 \pi} (A^4 \cos^4 \tau - 1) A \sin \tau \cos \tau d\tau \\
            &= 0
          \end{align*}

          Therefore, $\varphi = \alpha$ for some constant $\alpha$. This gives us that
          \[ y_0 = \frac{2^{3/4}}{(1 + C \exp(-2x))^{1/4}} \cos(t_1 + \alpha) .\]

          Therefore, we get the limit cycle being a circle of radius $2^{3/4}$.
      \end{enumerate}
  ...
  \item Holmes 3.35 \begin{problem}
    ...
    \begin{solution}
      ...
      \begin{align*}
        \frac{\partial^2 y_1}{\partial t_1^2} + y_1 &= \frac{1}{2} (D(\varepsilon t))^{-1/2} D'(\varepsilon t) \frac{\partial y_0}{\partial t_1} -
        2 \sqrt{D(\varepsilon t)} \frac{\partial^2 y_0}{\partial t_1 \partial t_2} + D'(\varepsilon t) \frac{1}{\sqrt{D(\varepsilon t)}}
        \frac{\partial y_0}{\partial t_1} \\
        &= \left( \frac{1}{2} D' (D(\varepsilon t))^{-1/2} + D'(\varepsilon t) (D(\varepsilon t))^{1/2} \right) (A_0(t_2) \cos t_1 - B_0(t_2) \sin t_1) \\
        &\quad -2 \sqrt{D(\varepsilon t)} \left( \frac{\partial A_0}{ \partial t_2} \cos t_1 - \frac{\partial B_0}{\partial t_2} \sin t_1 \right) \\
        &= \left( \frac{A_0}{2} D' D^{-1/2} + A_0 D' D^{1/2} - 2 D^{1/2} \frac{\partial A_0}{\partial t_2} \right) \cos t_1 \\
        &\quad - \left( \frac{B_0}{2} D' D^{-1/2} + B_0 D' D^{1/2} - 2 D^{1/2} \frac{\partial B_0}{\partial t_2} \right) \sin t_1
      \end{align*}

      In order to prevent secular terms, we want
      \[ \frac{A_0}{2} D' D^{-1/2} + A_0 D' D^{1/2} - 2 D^{1/2} \frac{\partial A_0}{\partial t_2}
      = \frac{B_0}{2} D' D^{-1/2} + B_0 D' D^{1/2} - 2 D^{1/2} \frac{\partial B_0}{\partial t_2} = 0 \]
  ...
  \item Holmes 4.19 \begin{problem}
    ...
    \begin{solution}
      \begin{enumerate}
        ...
        \item
          In part (a), we found our approximation to the time-independent Schr\"{o}dinger equation to be
          \[ \psi \sim \begin{cases}
              \frac{1}{|q(x)|^{1/4}} \left[ 2 a_R \cos \left( \frac{1}{\varepsilon} \theta_1(x) - \frac{\pi}{4} \right) + b_R \cos \left(
              \frac{1}{\varepsilon} \theta_1(x) + \frac{\pi}{4} \right) \right] &\text{if } x < a \\
              \frac{1}{q(x)^{1/4}} \left( a_R e^{-\kappa_1(x)/\varepsilon} + b_R e^{\kappa_1(x)/\varepsilon} \right) &\text{if } a < x < b \\
          ...
          For the time-dependent Schr\"{o}dinger equation, we are taking our solution to be $\Psi = \exp{-iEt} \psi(x)$. Therefore our solution to the
          left and right of the barrier will be given by
          \[ \Psi(x,t) \sim \begin{cases}
              \frac{\exp(-iEt)}{|q(x)|^{1/4}} \left[ 2 a_R \cos \left( \frac{1}{\varepsilon} \theta_1(x) - \frac{\pi}{4} \right) + b_R \cos \left(
              \frac{1}{\varepsilon} \theta_1(x) + \frac{\pi}{4} \right) \right] &\text{if } x < a \\
              \frac{\exp(-iEt)}{|q(x)|^{1/4}} \left[ 2 e^C b_R \cos \left( \frac{1}{\varepsilon} \kappa_2(x) - \frac{\pi}{4} \right) + e^{-C} a_R \cos \left(
              \frac{1}{\varepsilon} \kappa_2(x) + \frac{\pi}{4} \right) \right] &\text{if } b < x
          \end{cases} \]
      \end{enumerate}
    ...
    \begin{solution}
      ...
      \begin{enumerate}
          \item
            ...
            \theta/\varepsilon)} \sin(\lambda_n y) .\]

        \item
          The turning points we must be concerned with occur when $\theta_x = 0$.
          Assume $\theta_x(x_t) = 0$.
          By equation \eqref{eq:theta_x}, we get
          \[ \mu(x_t) = \frac{n \pi}{\omega} .\]
          Because $\mu' < 0$ we have $\frac{d}{dx} (\theta_x)^2 < 0$, so there is only one turning point (for each mode).

          Now we introduce the transition-layer coordinate
          \[ \bar{x} = \frac{x - x_t}{\varepsilon^\beta} .\]
          We denote the transition-layer solution as $U(\bar{x}, y, t)$ and assume that
          \[ U(\bar{x}, y, t) \sim \varepsilon^\gamma e^{i \omega t} \left( U_0(\bar{x}) \sin(\lambda_n y) + \dots \right) .\]
          Substituting these into our original differential equation gives
          \begin{align*}
          \varepsilon^{2-2\beta} U_0'' + \theta_x^2(x_t + \varepsilon^\beta \bar{x}) U_0 + \dots = 0
          \end{align*}

          Then
          \begin{align*}
            \theta_x^2(x_t + \varepsilon^\beta \bar{x}) &\sim \theta_x^2(x_t) + \varepsilon^\beta \bar{x} \frac{d}{dx}\theta_x^2(x_t) + \dots \\
            &\sim \varepsilon^\beta \bar{x} \frac{d}{dx} \theta_x^2(x_t) + \dots
          \end{align*}

          Plugging this in, we have the equation
          \[ \varepsilon^{2-2\beta} U_0'' + \varepsilon^\beta \bar{x} \frac{d}{dx}\theta_x^2(x_t) U_0 + \dots = 0 .\]

          To get terms to balance, we need $2-2\beta = \beta$. That is, we require $\beta = 2/3$. Then the $O(\varepsilon^{2/3})$ equation we are
          trying to solve is
          \[ U_0'' = \kappa \bar{x} U_0 \]
          with
          \begin{align*}
            \kappa &= -\frac{d}{dx} \theta_x^2(x_t) \\
            &= -2\omega^2 \mu \mu'
          \end{align*}

          This is again an Airy equation with solution given by
          \[ U_0 = a Ai(\kappa^{1/3}\bar{x}) + b Bi(\kappa^{1/3} \bar{x}) .\]
          Because we are only interested in bounded solutions, we know $b=0$. In order to find $a$, we would have to use the asymptotics of the Airy
          function to match the transition layer solution to the solution from part (a).
      \end{enumerate}
